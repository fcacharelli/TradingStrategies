...
def grid(df,grids,pct_range,wtime):
	...
	win=0
	trades=0
	for i in range(50*24,len(df)-wtime,24):
		entry_price=df['Close'].iloc[i]
		if entry_price<df['SMA'].iloc[i]:
...
data['SMA'] = data.Close.rolling(50*24).mean()

days = [1,2,5,10,15,30,45]
ranges = [5,10,15,20]
grids = [10,20,30,40,50,60,70,80]
f = open("myfile.csv", "a")
f.write('Day,Grid,Range,Trades,Wins\n')
for day in days:
	print(day)
	for gride in grids:
		print(gride)
		for rango in ranges:
			trades,wins = grid(data,gride,rango/100,day*24)
			f.write(str(day)+','+str(gride)+','+str(rango)+','+str(trades)+','+str(wins)+'\n')
f.close()
